...
import org.halalz.fenix.datastore.SelectedPocketStoriesCategories
import org.halalz.fenix.datastore.SelectedPocketStoriesCategories.SelectedPocketStoriesCategory
import org.halalz.fenix.home.pocket.PocketRecommendedStoriesCategory
import org.halalz.fenix.home.pocket.PocketRecommendedStoriesSelectedCategory
...
class PocketUpdatesMiddlewareTest {
    ...
    fun `WHEN PocketStoriesCategoriesChange is dispatched THEN intercept and dispatch PocketStoriesCategoriesSelectionsChange`() = runTestOnMain {
        ...
        val persistedSelectedCategories: org.halalz.fenix.datastore.SelectedPocketStoriesCategories = mockk {
            every { valuesList } returns mutableListOf(persistedSelectedCategory)
        }
        val dataStore: DataStore<org.halalz.fenix.datastore.SelectedPocketStoriesCategories> =
            mockk<FakeDataStore<org.halalz.fenix.datastore.SelectedPocketStoriesCategories>>(relaxed = true) {
                every { data } returns flowOf(persistedSelectedCategories)
            } as DataStore<org.halalz.fenix.datastore.SelectedPocketStoriesCategories>
    ...
    fun `WHEN SelectPocketStoriesCategory is dispatched THEN persist details in DataStore`() = runTestOnMain {
        ...
        val dataStore: DataStore<org.halalz.fenix.datastore.SelectedPocketStoriesCategories> =
            mockk<FakeDataStore<org.halalz.fenix.datastore.SelectedPocketStoriesCategories>>(relaxed = true) as
                DataStore<org.halalz.fenix.datastore.SelectedPocketStoriesCategories>
        val pocketMiddleware = PocketUpdatesMiddleware(mockk(), dataStore, this)
        val appStore = spyk(
            AppStore(
                AppState(
